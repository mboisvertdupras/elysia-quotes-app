...
var processRecordQueue = function() {
...
var initInterceptors2 = function(data2) {
...
async function send({ method, action, body, referrer, headers }, followRedirects) {
  let proxy;
  let handleRedirects = redirectHandler(followRedirects);
  let onSuccess = (response2) => response2;
  let onError = (error2) => error2;
  if (method === "GET") {
    ...
    onError = (error2) => dequeue(action, (job) => job.reject(error2));
  }
  ...
  let response = fetch(action, {
    ...
    method,
    body
  }).then(handleRedirects).then(readHtml).then(onSuccess).catch(onError);
  return method === "GET" ? proxy : response;
}
var readHtml = function(response) {
...
var dequeue = function(key, resolver) {
  (jobs[key] || []).forEach(resolver);
  jobs[key] = undefined;
};
var redirectHandler = function(follow) {
...
var morph = function(from, toHtml, options) {
...
var shouldSkip = function(hook, ...args) {
  let skip = false;
  hook(...args, () => skip = true);
  return skip;
};
...
var textOrComment = function(el) {
  return el.nodeType === 3 || el.nodeType === 8;
};
var getFirstNode = function(parent) {
  return parent.firstChild;
};
var getNextSibling = function(parent, reference) {
...
var monkeyPatchDomSetAttributeToAllowAtSymbols = function() {
  if (patched)
    return;
  patched = true;
...
var Ajax = function(Alpine2) {
  window.addEventListener("popstate", (e) => {
    if (!e.state || !e.state.__AJAX__)
      return;
    window.location.reload(true);
  });
  Alpine2.directive("target", (el, { modifiers, expression }, { evaluate: evaluate2, cleanup: cleanup2 }) => {
    let config = {
    ...
    config.headers = evaluate2(Alpine2.bound(el, "x-headers", "{}"));
    if (isLocalLink(el)) {
      cleanup2(listenForNavigate(el, config));
    } else if (isForm(el)) {
      cleanup2(listenForSubmit(el, config));
    }
  });
  Alpine2.addInitSelector(() => `[${Alpine2.prefixed("merge")}]`);
  Alpine2.directive("merge", (el, { modifiers, expression }) => {
    mergeConfig.set(el, {
      strategy: expression,
      transition: transition2(modifiers)
    });
  });
  Alpine2.magic("ajax", (el) => {
    return (action, options = {}) => {
      ...
      if (!headers) {
        headers = el.hasAttribute("x-headers") ? Alpine2.evaluate(el, Alpine2.bound(el, "x-headers", "{}")) : {};
      }
      let request = {
        action,
        method,
...
var parseIds = function(el, expression = null) {
  let ids = [el.id];
  if (expression) {
...
var parseModifiers = function(modifiers = []) {
  let followRedirects = globalConfig.followRedirects ? !modifiers.includes("nofollow") : modifiers.includes("follow");
  let history = false;
  if (modifiers.includes("push"))
    history = "push";
...
var listenForNavigate = function(el, config) {
  let handler4 = async (event) => {
    ...
    try {
      ...
      if (error2 instanceof FailedResponseError) {
  ...
  el.addEventListener("click", handler4);
  return () => el.removeEventListener("click", handler4);
};
var navigateRequest = function(link) {
  return {
    method: "GET",
    action: link.href,
...
var listenForSubmit = function(el, config) {
  let handler4 = async (event) => {
    ...
    try {
      ...
      if (error2 instanceof FailedResponseError) {
        console.warn(error2.message);
        el.removeEventListener("submit", handler4);
  ...
  el.addEventListener("submit", handler4);
  return () => el.removeEventListener("submit", handler4);
};
var formRequest = function(form, submitter = null) {
...
var mergeBodyIntoAction = function(body, action) {
...
async function render(request, targets, el, config) {
  let dispatch2 = () => true;
  if (config.events) {
    dispatch2 = (el2, name, detail) => {
      return el2.dispatchEvent(new CustomEvent(name, {
        detail,
        bubbles: true,
        composed: true,
        cancelable: true
...
async function merge(strategy, from, to) {
  let strategies = {
    ...
    morph(from2, to2) {
      morph(from2, to2);
      return document.getElementById(to2.id);
    }
  };
  ...
  let freshEl = null;
  let transition22 = document.startViewTransition(() => {
    freshEl = strategies[strategy](from, to);
    return Promise.resolve();
  });
  await transition22.updateCallbackDone;
  return freshEl;
}
...
var updateHistory = function(strategy, url) {
  let strategies = {
...
var addSyncTargets = function(targets) {
...
var sendConfig = new WeakMap;
var mergeConfig = new WeakMap;
Ajax.configure = (options) => {
  globalConfig = Object.assign(globalConfig, options);
  return Ajax;
};
var src_default2 = Ajax;
